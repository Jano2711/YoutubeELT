def test_api_key(api_key):
    """Test to verify the API key fixture."""
...
def test_dags_integrity(dagbag):
    ...
    for dag_id in expected_dags_ids:
        assert dag_id in loaded_dags_ids, f"DAG '{dag_id}' is missing in the DagBag."
    ...
    for dag_id, dag in dagbag.dags.items():
        ...
        assert (
            expected_count == actual_task_count
            ), f"DAG '{dag_id}' has {actual_task_count} tasks; expected {expected_count}."
